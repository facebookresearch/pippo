...
def match_features(img1, img2, conf_thresh=0.2, matches_thresh=5, num_features=2048):
    ...
    with th.inference_mode():
        ...
        features1, features2 = registry.disk(
        ...
        lafs1 = KF.laf_from_center_scale_ori(
            kps1[None], th.ones(1, len(kps1), 1, 1, device=pimg1.device)
        )
        lafs2 = KF.laf_from_center_scale_ori(
            kps2[None], th.ones(1, len(kps2), 1, 1, device=pimg2.device)
        )
...
def get_l2_reproj(video_grid, metrics_dict, batch, output_dir, sample_name, H, W, NV):
    ...
    row_view_grid = rearrange(
        video_grid, "B T (NR H) (NC W) C -> (B T NR) NC H W C", H=H, W=W
    )
    ...
    for row_idx, row in tqdm(
        enumerate(row_view_grid),
        total=len(row_view_grid),
        desc="computing reprojection error",
    ):
        ...
        for pair in pairs:
            ...
            save_path = os.path.join(
                output_dir,
                f"{sample_name}_row_{row_idx}_matches_{pair[0]}_{pair[1]}.png",
            )
            ...
            if "keypoints_3d" in batch and row_idx == 0:
                ...
                gt_keypoints_inds = gt_keypoints3d[:, :, 0].astype(
                    int
                )  # original indices out of 274
                gt_keypoints3d = gt_keypoints3d[:, :, 1:4]  # 3d keypoints
                gt_keypoints3d = np.append(
                    gt_keypoints3d, np.ones_like(gt_keypoints3d)[:, :, :1], axis=-1
                )  # homogenous coords

                # project to 2d
                gt_keypoints = [
                    np.dot(cam_krt, np.transpose(gt_kpts3d))
                    for gt_kpts3d, cam_krt in zip(gt_keypoints3d, cam_krts)
                ]
                gt_keypoints = [
                    gt_kpts2d[:2] / gt_kpts2d[-1] for gt_kpts2d in gt_keypoints
                ]
                ...
                gt_keypoints1 = [
                    cv2.KeyPoint(x, y, 1) for (x, y, c) in gt_keypoints[pair[0]]
                ]
                gt_keypoints2 = [
                    cv2.KeyPoint(x, y, 1) for (x, y, c) in gt_keypoints[pair[1]]
                ]

                gt_matches = [cv2.DMatch(i, i, 0) for i in range(len(gt_keypoints[0]))]
                gt_fil_inds = [
                    (
                        gt_keypoints[pair[0]][i][2] > 0
                        and gt_keypoints[pair[1]][i][2] > 0
                    )
                    for i in range(len(gt_keypoints[0]))
                ]
                gt_matches = np.array(gt_matches)[gt_fil_inds].tolist()

                gt_matches_img = cv2.drawMatches(
                    img1, gt_keypoints1, img2, gt_keypoints2, gt_matches, None, flags=2
                )
                gt_save_path = os.path.join(
                    output_dir, f"gt_row_{row_idx}_matches_{pair[0]}_{pair[1]}.png"
                )
            ...
            kpts_1_reproj_error = np.linalg.norm(
                kpts1_reprojected[:2] - tkpts1, axis=0
            ).mean()

            kpts2_reprojected = krt2 @ triangulated_points
            kpts2_reprojected /= kpts2_reprojected[2]
            kpts_2_reproj_error = np.linalg.norm(
                kpts2_reprojected[:2] - tkpts2, axis=0
            ).mean()
        ...
        for cam in sample["id"]["cams"][0]:
            ...
            for _cam in cameras["KRT"]:
                if str(cam) == _cam["cameraId"]:
                    ...
                    K = np.concatenate(
                        [np.array(_cam["K"]).T, np.zeros((3, 1))], axis=-1
                    )
